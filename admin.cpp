    ...
    ui(new Ui::admin)
{
    ui->setupUi(this);
    QSqlQuery query;
       query.exec("select * from carmodel");
       while(query.next())
       {
...
{
    QSqlQuery query;
       query.exec("select carphoto from carmodel where cartypeid='1' ");
        ...
        {
            QPixmap photo;
            photo.loadFromData(query.value(0).toByteArray(), "jpg"); //从数据库中读出图片为二进制数据，图片格式为png，然后显示到QLabel里
